...
```
ls f TAB d TAB A 0 6 TAB 1 TAB
```
...
#I don't know why it is saying permission denied. When I run this code in the terminal, it works.
```
...
```{bash}
ls fastq/DPCh_plate1_*.R1.fq.gz
ls fastq/DPCh_plate1_*.R1.fq.gz | wc
cp fastq/DPCh_plate1_*.R1.fq.gz lonely-fastqs/R1
ls fastq/DPCh_plate1_*.R2.fq.gz
ls fastq/DPCh_plate1_*.R2.fq.gz | wc
```
...
```{bash}
ls fastq/DPCh_plate1_[A-B]*.R1.fq.gz
ls fastq/DPCh_plate1_[A-B]*.R1.fq.gz | wc
...
```{bash}
rm -r bad\ directory\ with\ spaces/
rm -r  nice_dir_name/
...
```
To make sure that you don't accidentally remove something you don't want to remove. 
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
5 directories named each of those words (i.e., "bad", "directory", etc.).
```
...
```{bash}
zcat fastq/DPCh_plate1_C11_S35.R1.fq.gz  | head -n8
```

# Do the same to see the first 8 lines of `fastq/DPCh_plate1_C11_S35.R2.fq.gz`

```{bash}
zcat fastq/DPCh_plate1_C11_S35.R2.fq.gz  | head -n8
```
...
```{bash}
zcat fastq/DPCh_plate1_*R1.fq.gz | wc
```
...
```{bash}
zcat fastq/DPCh_plate1_*R1.fq.gz > R1_all_via_gzcat.fq
```
...
```{bash}
zcat ./fastq/DPCh_plate1_A05_S5.R1.fq.gz | head -n 10
cat fastq/DPCh_plate1_*R1.fq.gz > R1_all_via_cat.fq.gz


#zcat is if you want a read a gzipped file in terminal window
```
...
```{bash}
gzip R1_all_via_gzcat.fq #once you gzip it, it adds .gz
...
tail sam/DPCh_plate1_A05_S5.sam -n15
...
head sam/DPCh_plate1_A05_S5.sam -n15
...
```{bash}
#I didnt get that error, even though below it doesnt show an x. I'm not quite sure how to do this properly.

ls -l count_files.sh
chmod 744 count_files.sh
#or
chmod cicch+x count_files.sh
#?
```
