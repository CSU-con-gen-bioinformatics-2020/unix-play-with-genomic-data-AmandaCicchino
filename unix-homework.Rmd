...
```
I'm not sure. 5 directories named each of those words (i.e., "bad", "directory", etc.)?
```
...
```{bash}
zcat fastq/DPCh_plate1_C11_S35.R1.fq.gz  | head
```

# Do the same to see the first 8 lines of `fastq/DPCh_plate1_C11_S35.R2.fq.gz`

```{bash}
zcat fastq/DPCh_plate1_C11_S35.R2.fq.gz  | head
```
